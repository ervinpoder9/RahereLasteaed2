...
public abstract class AssemblyTests(string namespaceName) : StaticTests {
    ...
    [TestMethod] public override void IsTested() {
        ...
        if (namespaceName.StartsWith("Mvc."))
            ...
            testNamespace = "Mvc.Tests." + namespaceName;
        ...
        if (assembly == null)
            assembly = assemblies.FirstOrDefault(a => a.FullName?.StartsWith(assemblyName) ?? false);
        if (assembly == null)
            notTested($"Assembly {namespaceName} not found.");
